...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function exportCuts(fmt, label) {
        ...
        showCutMessage(`${label} wird exportiert...`, "");
        if (window.pywebview && window.pywebview.api && window.pywebview.api.export_cuts) {
            const cutsData = cuts.map(c => [c.start, c.end]);
            window.pywebview.api.export_cuts(currentPlayingFile.path, JSON.stringify(cutsData), fmt).then(result => {
                if (result && result.success) {
                    showCutMessage(`${label} exportiert: ${result.filename}`, "success");
                    announce(`${label} erfolgreich exportiert unter ${result.filename}`);
                } else {
                    showCutMessage(`Fehler beim ${label}-Export: ${result.error || 'Abgebrochen'}`, "error");
                }
            }).catch(err => {
    ...
    if (btnExportEdl) btnExportEdl.addEventListener('click', () => exportCuts('edl', 'EDL'));
    const btnExportFcpxml = document.getElementById('btn-export-fcpxml');
    if (btnExportFcpxml) btnExportFcpxml.addEventListener('click', () => exportCuts('fcpxml', 'FCPXML'));
